  ...
  usersModel.findOne({ email: userInfo.email }, function (error, oldUser) {
    if (error) {
      ...
      if (
        ...
        return res.send({
          status: "ok",
          msg: `The following email address ${userInfo.email} is already registered in our database.`,
        });
      } else {
        ...
        newUser.save(function (error) {
          ...
          res.send({
            status: "ok",
            msg: "Thank you for subscribing! Welcome to the club!",
          });
        });
      }
    }
  ...
  usersModel.findOne({ email: userInfo.email }, function (error, oldUser) {
    if (error) {
      ...
      if (
        ...
        return res.send({
          status: "ok",
          msg: `The email ${oldUser.email} is already registered in our database.`,
        });
      } else {
        const newUser = new usersModel({
          first: userInfo.firstName,
...
users.post("/login", async function (req, res) {
  ...
  if (!exists) {
    return res.status(401).json({
      status: "error",
      msg: `The email ${user.email} does not appear in our database.`,
    });
  }
  // Compares password
  ...
  if (passOK === true) {
    ...
    return res
      .status(200)
      .json({ status: "ok", msg: "Logged in", token, url: "/account" });
  }
  return res
    .status(401)
    .json({ status: "error", msg: "ERROR: Wrong Credentials. 2" });
});
